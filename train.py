...
if __name__ == '__main__':
    ...
    my_batch_sampler = MyBatchSampler(text_superset, text2image_indices_dict, num_batches, num_batch_size)
    ...
    val_batch_sampler = ValBatchSampler(text_superset, val_dict_index, num_batches, num_batch_size)
    val_dataloader = DataLoader(dataset=val_dataset, batch_sampler=val_batch_sampler, shuffle=False, **kwargs)    
    # indices = list(range(len(val_dataset)))
    # subset_size = int(len(val_dataset) * 0.7)
    # val_indices = random.sample(indices, subset_size)
    ...
    for epoch in range(num_epoch):
        ...
        for batch in tqdm(train_dataloader,total=num_batches, desc=f'epoch : {epoch + 1}/{num_epoch}'):
        ...
        with torch.no_grad():
            for batch in tqdm(val_dataloader, total=num_batches, desc=f'val_epoch : {epoch + 1}/{num_epoch}'):
