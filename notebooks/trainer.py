...
def show_images(images, nrow=8):
    ...
    # 이미지를 그리드로 표시
    grid_img = grid_img = utils.make_grid(images.cpu(), nrow=nrow, padding=2)
...
class Dataset(Dataset):
    ...
    def __init__(
        ...
        f = os.popen("ls " + folder)
        _p = f.read()
        self._paths = list(_p.split('\n'))
        self._paths.remove('')

        self.paths = []
        for _path in self._paths:
            paths_c = [p for ext in exts for p in Path(f"{folder}/{_path}/{self.path_postfix}").glob(f"**/*.{ext}")]
            self.paths.extend(paths_c)
...
class DiffusionCIFAR10(torchvision.datasets.CIFAR10):
    def __init__(
        self,
        root: str,
        train: bool = True,
...
class Trainer(object):    
    ...
    def __init__(
        ...
        *,
        ...
        sys.stderr.write(str(self.device))
        ...
        if dataset_name == "cifar10":
            ...
            self.ds = DiffusionCIFAR10(folder, train=True, transform=train_transform)
        else:
            self.ds = Dataset(
                folder,
                self.image_size,
    ...
    def sample_images_at_milestone(self, milestone, save = 1):
        # milestone에서의 모델 상태 불러오기
        self.load(milestone)
        ...
        if (save == 1):
            ...
            utils.save_image(
                all_images,
                str(self.results_folder / f"sample_at_milestone_{milestone}.png"),
                nrow=int(math.sqrt(self.num_samples)),
            )
